...
import com.liang.content.service.ContentService;
import com.liang.manager.dto.ContentDto;
import com.liang.manager.dto.DtoUtil;
import com.liang.manager.dto.front.*;
...
public class ContentServiceImpl implements ContentService {
    ...
    public int addContent(TbContent tbContent) {
        ...
        if(tbContentMapper.insert(tbContent)!=1){
            throw new UMallException("添加内容失败");
        }
        //同步缓存
        deleteHomeRedis();
        return 1;
    ...
    public int deleteContent(Long id) {
        ...
        if(tbContentMapper.deleteByPrimaryKey(id)!=1){
            throw new UMallException("删除内容失败");
        }
        //同步缓存
        deleteHomeRedis();
        return 1;
    ...
    public int updateContent(TbContent tbContent) {
        ...
        if(tbContentMapper.updateByPrimaryKey(tbContent)!=1){
            throw new UMallException("更新内容失败");
        }
        //同步缓存
        deleteHomeRedis();
        return 1;
    ...
    public TbContent getContentById(Long id) {
        ...
        if(tbContent==null){
            throw new UMallException("通过id获取内容失败");
        }
        return tbContent;
    }
