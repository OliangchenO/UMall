...
import com.liang.common.utils.IDUtil;
import com.liang.manager.dto.DtoUtil;
import com.liang.manager.dto.ItemDto;
import com.liang.manager.mapper.TbItemCatMapper;
import com.liang.manager.mapper.TbItemDescMapper;
import com.liang.manager.mapper.TbItemMapper;
import com.liang.manager.pojo.TbItem;
import com.liang.manager.pojo.TbItemCat;
import com.liang.manager.pojo.TbItemDesc;
import com.liang.manager.pojo.TbItemExample;
import com.liang.manager.service.ItemService;
...
public class ItemServiceImpl implements ItemService {
    ...
    public TbItem alertItemState(Long id, Integer state) {
        ...
        if (tbItemMapper.updateByPrimaryKey(tbMember) != 1){
            throw new UMallException("修改商品状态失败");
        }
    ...
    public int deleteItem(Long id) {
        ...
        if(tbItemMapper.deleteByPrimaryKey(id)!=1){
            throw new UMallException("删除商品失败");
        }
        if(tbItemDescMapper.deleteByPrimaryKey(id)!=1){
            throw new UMallException("删除商品详情失败");
        }
        //发送消息同步索引库
    ...
    public TbItem addItem(ItemDto itemDto) {
        ...
        if(tbItemMapper.insert(tbItem)!=1){
            throw new UMallException("添加商品失败");
        }
        ...
        if(tbItemDescMapper.insert(tbItemDesc)!=1){
            throw new UMallException("添加商品详情失败");
        }
        //发送消息同步索引库
    ...
    public TbItem updateItem(Long id,ItemDto itemDto) {
        ...
        if(tbItemMapper.updateByPrimaryKey(tbItem)!=1){
            throw new UMallException("更新商品失败");
        }
        ...
        if(tbItemDescMapper.updateByPrimaryKey(tbItemDesc)!=1){
            throw new UMallException("更新商品详情失败");
        }
        //同步缓存
