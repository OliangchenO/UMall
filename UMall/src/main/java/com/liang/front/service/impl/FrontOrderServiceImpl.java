...
public class FrontOrderServiceImpl implements FrontOrderService {
    ...
    public Order getOrder(Long orderId) {
        ...
        if(tbOrder==null){
            throw new UMallException("通过id获取订单失败");
        }
    ...
    public int cancelOrder(Long orderId) {
        ...
        if(tbOrderMapper.updateByPrimaryKey(tbOrder)!=1){
            throw new UMallException("取消订单失败");
        }
        return 1;
    }
    ...
    public Long createOrder(OrderInfo orderInfo) {
        ...
        if(member==null){
            throw new UMallException("获取下单用户失败");
        }
        ...
        if(tbOrderMapper.insert(order)!=1){
            throw new UMallException("生成订单失败");
        }
        ...
        for(CartProduct cartProduct:list){
            ...
            if(tbOrderItemMapper.insert(orderItem)!=1){
                throw new UMallException("生成订单商品失败");
            }
        ...
        if(tbOrderShippingMapper.insert(orderShipping)!=1){
            throw new UMallException("生成物流信息失败");
        }
    ...
    public int delOrder(Long orderId) {
        ...
        if(tbOrderMapper.deleteByPrimaryKey(String.valueOf(orderId))!=1){
            throw new UMallException("删除订单失败");
        }
    ...
    public String judgeOrder(TbOrder tbOrder){
        ...
        if(tbOrder.getStatus()==0){
            ...
            if(days>=1){
                ...
                tbOrder.setCloseTime(new Date());
                if(tbOrderMapper.updateByPrimaryKey(tbOrder)!=1){
                    throw new UMallException("更新订单失效失败");
                }
            }else {
                //返回到期时间
